...
export default function QualitySystem() {
  ...
  const statusColor = (s) => s === 'implemented' ? 'bg-emerald-500/20 text-emerald-400' : s === 'in_progress' ? 'bg-amber-500/20 text-amber-400' : 'bg-slate-700 text-slate-400';
    ...
    <div>
      <div className="flex items-center justify-between mb-2">
        <div>
          ...
          <h1 className="text-3xl font-semibold tracking-tight text-white">Build your ISQM-1 system</h1>
        ...
        <div className="space-y-2">
          ...
          {components.map(c => (
            <button key={c.id} onClick={() => { setSelected(c); setShowAdd(false); }}
              className={`w-full rounded-2xl border p-3 text-left transition ${selected?.id === c.id ? 'border-amber-500/50 bg-slate-800 shadow-sm' : 'border-slate-800 hover:bg-slate-800/50'}`}>
              <div className="text-[10px] font-bold text-slate-600 uppercase tracking-wider">C{c.order_index}</div>
              <div className="font-medium text-slate-200 mt-1 text-sm leading-snug">{c.name}</div>
            </button>
          ))}
        </div>
        ...
        <Card className="rounded-2xl border-slate-700 bg-slate-900/50">
          <CardHeader className="border-b border-slate-800">
            <div className="flex items-center justify-between">
              <div>
                <CardTitle className="text-white">{selected?.name || 'Select a component'}</CardTitle>
          ...
          <CardContent className="p-0">
            {showAdd && (
              <div className="p-4 border-b border-slate-800 bg-slate-800/30 space-y-3">
                ...
                <div>
                  ...
                  <Input className="mt-1 rounded-xl bg-slate-800 border-slate-700 text-white placeholder:text-slate-600" placeholder="e.g. Personnel fulfil responsibilities for ethical requirements including independence" value={newObj.title} onChange={e => setNewObj({ ...newObj, title: e.target.value })} />
                  <HelperText>What must the firm achieve? Write one clear sentence.</HelperText>
                </div>
                <div>
                  <FieldLabel label="Description" />
                  <Textarea className="mt-1 rounded-xl bg-slate-800 border-slate-700 text-white placeholder:text-slate-600 min-h-16" placeholder="Additional context about what this objective means for CLA Romania..." value={newObj.description} onChange={e => setNewObj({ ...newObj, description: e.target.value })} />
                  <HelperText>Optional. Explain scope and what "achieved" looks like.</HelperText>
                </div>
                <div className="flex gap-3 items-center">
                  <div>
                    <FieldLabel label="Status" />
                    <select className="mt-1 rounded-xl bg-slate-800 border border-slate-700 text-white px-3 py-2 text-sm" value={newObj.status} onChange={e => setNewObj({ ...newObj, status: e.target.value })}>
            ...
            {objectives.map(obj => (
              <div key={obj.id} className="p-4 border-b border-slate-800 last:border-b-0 hover:bg-slate-800/30 transition group">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1">
                    <div className="font-medium text-slate-200">{obj.title}</div>
                  ...
                  <div className="flex items-center gap-2">
                    <select className="text-xs border border-slate-700 rounded-lg px-2 py-1 bg-slate-800 text-slate-300" value={obj.status} onChange={e => updateStatus(obj.id, e.target.value)}>
                      <option value="draft">Draft</option><option value="in_progress">In progress</option><option value="implemented">Implemented</option>
                    </select>
